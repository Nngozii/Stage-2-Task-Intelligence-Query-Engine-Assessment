...
app.get("/api/profiles/search", async (req, res) => {
  ...
  // Gender
  if (/\bmales?\b/.test(query) && !/\bfemales?\b/.test(query)) filters.gender = "male";
  ...
  else if (/\byoung\b/.test(query)) { filters.min_age = 16; filters.max_age = 24; }
  ...
  const countryMap = {
    ...
    "cameroon": "CM", "zimbabwe": "ZW", "zambia": "ZM", "mali": "ML",
    "niger": "NE", "chad": "TD", "sudan": "SD", "somalia": "SO",
    "rwanda": "RW", "benin": "BJ", "togo": "TG", "guinea": "GN",
    "mozambique": "MZ", "madagascar": "MG", "ivory coast": "CI",
    "south africa": "ZA", "egypt": "EG", "morocco": "MA", "algeria": "DZ",
    "tunisia": "TN", "libya": "LY", "congo": "CG", "drc": "CD",
    "burkina faso": "BF", "malawi": "MW", "namibia": "NA", "botswana": "BW",
  ...
  const fromMatch = query.match(/from\s+([a-z\s]+?)(?:\s+(?:above|below|older|younger|between|who|aged|$))/);
  ...
  if (filters.gender) { conditions.push(`gender = $${i++}`); params.push(filters.gender); }
  if (filters.age_group) { conditions.push(`age_group = $${i++}`); params.push(filters.age_group); }
  if (filters.country_id) { conditions.push(`country_id = $${i++}`); params.push(filters.country_id); }
  if (filters.min_age !== undefined) { conditions.push(`age >= $${i++}`); params.push(filters.min_age); }
  if (filters.max_age !== undefined) { conditions.push(`age <= $${i++}`); params.push(filters.max_age); }
  ...
  const countResult = await pool.query(`SELECT COUNT(*) FROM profiles ${where}`, params);
  const total = parseInt(countResult.rows[0].count);
